...
export const removeItemFromCart = createAsyncThunk(
  ...
  async (agrs: { productId: string; size: string; token: string | null }) => {
    try {
      let config = {}
      ...
      if (agrs.token) {
        config = {
          headers: {
            Authorization: `Bearer ${agrs.token}`,
          },
        }
      }

      const response = await privateAxios.delete(
        `${CART_SERVICE_PATH}/items/${agrs.productId}/size/${agrs.size}`,
        config,
      )
...
export const {increaseQuantityByOne} = cartSlice.actions
